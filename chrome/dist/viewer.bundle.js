(() => {
  ...
  var filterTimer = 0;
  var analysisStatusTimer = 0;
  var virtualScrollFrame = 0;
  ...
  function saveFilterHistory() {
    state.filterHistory = state.filterHistory.slice().sort((left, right) => right.createdAt - left.createdAt).slice(0, 20);
  ...
  function rememberFilterHistory(filterText, title = "") {
    ...
    const recentHistory = state.filterHistory.slice().sort((left, right) => right.createdAt - left.createdAt).slice(0, 20);
    state.filterHistory = [
      {
  ...
  function findMatchingRows(query) {
    ...
    return state.visibleRows.map((row, index) => ({ row, index })).filter(({ row }) => row.searchable.toLowerCase().includes(lowerQuery));
  }
  ...
  function goModalMatch(direction) {
    ...
    state.modalActiveMatch = state.modalActiveMatch < 0 ? direction > 0 ? 0 : state.modalMatches.length - 1 : (state.modalActiveMatch + direction + state.modalMatches.length) % state.modalMatches.length;
  ...
  function updateCompareModalHeader() {
    ...
    const leftText = leftTab ? `${leftTab.fileName} \xB7 ${leftTab.rows.length.toLocaleString()} ${state.language === "en" ? "rows" : "\u884C"}` : t("selectLog");
    const rightText = rightTab ? `${rightTab.fileName} \xB7 ${rightTab.rows.length.toLocaleString()} ${state.language === "en" ? "rows" : "\u884C"}` : t("selectLog");
  ...
  function renderCompareRows(side) {
    ...
    for (let index = start; index < end; index += 1) {
      ...
      fragment.appendChild(state.compareLineMode ? createCompareAlignedLogRow(row, side, filterRules) : createCompareLogRow(
        row,
        side,
  ...
  function alignCompareRange(leftRows, rightRows, leftStart, leftEnd, rightStart, rightEnd, output) {
    while (leftStart < leftEnd && rightStart < rightEnd && normalizeCompareLine(leftRows[leftStart].raw) === normalizeCompareLine(rightRows[rightStart].raw)) {
      output.push({ type: "same", left: leftRows[leftStart], right: rightRows[rightStart] });
      leftStart += 1;
      rightStart += 1;
    }
    const suffix = [];
    while (leftStart < leftEnd && rightStart < rightEnd && normalizeCompareLine(leftRows[leftEnd - 1].raw) === normalizeCompareLine(rightRows[rightEnd - 1].raw)) {
  ...
  function findPatienceAnchors(leftRows, rightRows, leftStart, leftEnd, rightStart, rightEnd) {
    const leftCounts = /* @__PURE__ */ new Map();
  ...
  function alignCompareChangedBlock(leftRows, rightRows, leftStart, leftEnd, rightStart, rightEnd, output) {
    ...
    if (leftCount * rightCount <= 2e5 && Math.max(leftCount, rightCount) <= 1e3) {
  ...
  function alignCompareSmallBlock(leftRows, rightRows, leftStart, leftEnd, rightStart, rightEnd, output) {
    ...
    for (let left2 = leftCount - 1; left2 >= 0; left2 -= 1) {
      for (let right2 = rightCount - 1; right2 >= 0; right2 -= 1) {
        if (leftKeys[left2] === rightKeys[right2]) {
          dp[left2][right2] = dp[left2 + 1][right2 + 1] + 1;
        } else {
          dp[left2][right2] = Math.max(dp[left2 + 1][right2], dp[left2][right2 + 1]);
        }
      }
    }
    let left = 0;
  ...
  async function importFilterLogFile(file) {
    ...
    if (!filterText) {
      showToast(state.language === "en" ? "No filter logs found in this file." : "\u6587\u4EF6\u4E2D\u672A\u627E\u5230\u8FC7\u6EE4\u65E5\u5FD7\u3002");
      return;
    }
    ...
    applyFilter();
    closeImportModal();
    showToast(state.language === "en" ? `Imported filter logs: ${file.name}` : `\u5DF2\u5BFC\u5165\u8FC7\u6EE4\u65E5\u5FD7\uFF1A${file.name}`);
  }
  function openBreakpointsModal() {
    ...
    els.breakpointsModalMeta.textContent = `${state.breakpointsFileName || "\u65AD\u70B9\u6587\u4EF6"} \xB7 ${state.breakpointsContent.length.toLocaleString()} \u5B57\u7B26`;
...
<\/script>`
  ...
  function renderBreakpointAnalysis(breakpoints, query) {
    ...
    const logStrings = Array.isArray(breakpoints.logStrings) ? breakpoints.logStrings.filter((value) => typeof value === "string" && value) : [];
    const matchedLogs = Array.isArray(breakpoints.matchedLogs) ? breakpoints.matchedLogs : [];
    const matchedByLogString = /* @__PURE__ */ new Map();
  ...
  function goAnalysisModalMatch(direction) {
    ...
    if (state.analysisRightRows.length && state.analysisRightBody) {
      state.analysisModalActiveMatch = state.analysisModalActiveMatch < 0 ? direction > 0 ? 0 : state.analysisModalMatches.length - 1 : (state.analysisModalActiveMatch + direction + state.analysisModalMatches.length) % state.analysisModalMatches.length;
    ...
    state.analysisModalActiveMatch = (state.analysisModalActiveMatch + direction + state.analysisModalMatches.length) % state.analysisModalMatches.length;
  ...
  function getAnalysisRightPaneText() {
    if (state.analysisRightRows.length) {
      return state.analysisRightRows.map((row) => `${row.lineNumber}: ${row.lineText}`).join("\n");
    }
