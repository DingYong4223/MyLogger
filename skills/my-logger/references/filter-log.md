...
osascript -e 'tell application "Terminal" to activate' \
  -e "tell application \"Terminal\" to do script \"nvim +'set number' +'set norelativenumber' +'syntax on' +'source $syntax_file' '$filtered'\""
```
...
filtered="$(mktemp -t mylogger-filtered-log.XXXXXX)"
rg -n -- "<keyword>" "<log-path>" > "$filtered" || true
nvim +'set number' +'set norelativenumber' +'syntax on' +"source $syntax_file" "$filtered"
```
...
6. Open a new terminal window and run `nvim +'set number' +'set norelativenumber' +'syntax on' +'source <syntax-file>' <filtered-file>`.
